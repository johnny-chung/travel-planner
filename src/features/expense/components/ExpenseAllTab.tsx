        ...
        <div className="space-y-2">
          {expenses.map((expense) => (
            <div key={expense._id} className="bg-card rounded-2xl p-4 shadow-sm border border-border flex items-center gap-3">
              <div className="shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-base" title={expense.type === "own" ? "Personal" : "Shared"}>
                {expense.type === "own" ? "👤" : "🤝"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-foreground text-sm truncate">{expense.description}</p>
                <p className="text-muted-foreground text-xs mt-0.5">
                  {format(new Date(expense.date), "MMM d, yyyy")} · by {getMemberName(expense.addedBy)}
                </p>
              </div>
